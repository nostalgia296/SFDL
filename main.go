...
func init() {
	...
	specialRunes := []rune(SPECIAL_CHARS)
	replaceRunes := []rune(REPLACEMENT_CHARS)
	for i := 0; i < len(specialRunes) && i < len(replaceRunes); i++ {
		charMap[specialRunes[i]] = replaceRunes[i]
	}
}
...
func initNonce() {
	for {
		...
		sign := getSign(nonce, timestamp, DeviceToken)
		headers["sfsecurity"] = fmt.Sprintf("nonce=%s&timestamp=%d&devicetoken=%s&sign=%s", nonce, timestamp, DeviceToken, sign)
		headers["user-agent"] = fmt.Sprintf("boluobao/5.1.54(android;35)/OPPO/%s/OPPO", strings.ToLower(DeviceToken))

		resp, err := request("GET", fmt.Sprintf("%s/Chaps/8436696?expand=content%%2Cexpand.content", BaseURL), nil)
		if err != nil {
			continue
		}

		if httpCode := getHTTPCode(resp); httpCode != 417 {
			break
		}
...
// 获取小说信息
func getCatalog(novelID string) (string, string, string, []Volume) {
	var volumes []Volume
	...
	headers["sfsecurity"] = fmt.Sprintf("nonce=%s&timestamp=%d&devicetoken=%s&sign=%s", nonce, timestamp, DeviceToken, sign)
	//获取标题
	resp, err := request("GET", fmt.Sprintf("%s/novels/%s?expand=bigNovelCover", BaseURL, novelID), nil)
	if err != nil || getHTTPCode(resp) != 200 {
		fmt.Println("标题获取失败")
		return "标题获取失败", "", "", nil
	}

	if data, ok := resp["data"].(map[string]interface{}); ok {
		title = getString(data, "novelName")
		author = getString(data, "authorName")
		if expand, ok := data["expand"].(map[string]interface{}); ok {
			cover = getString(expand, "bigNovelCover")
		}
	}
	...
	resp, err = request("GET", fmt.Sprintf("%s/novels/%s/dirs?expand=originNeedFireMoney", BaseURL, novelID), nil)
	if err != nil || getHTTPCode(resp) != 200 {
		fmt.Println("目录获取失败")
		return "目录获取失败", "", "", nil
	}
...
// 下载章节
func downloadChapter(chapterID string) (*Chapter, error) {
	var chap Chapter
	...
	for t := 0; t < 5; t++ {
		...
		resp, err := request("GET", fmt.Sprintf("%s/Chaps/%s?expand=content%%2Cexpand.content", BaseURL, chapterID), nil)
		if err != nil {
			time.Sleep(500 * time.Millisecond)
			continue
		}

		code := getHTTPCode(resp)
		if code == 200 {
			if data, ok := resp["data"].(map[string]interface{}); ok {
				chap.Title = getString(data, "title")
				chap.ID = chapterID
				...
				// 字符替换
				chap.Content = replaceChars(content)
				return &chap, nil
			}
		} else if code == 403 {
...
// 登录获取 cookie
func getCookie(username, password string) (string, error) {
	timestamp := time.Now().UnixMilli()
	sign := getSign(nonce, timestamp, DeviceToken)
	headers["sfsecurity"] = fmt.Sprintf("nonce=%s&timestamp=%d&devicetoken=%s&sign=%s", nonce, timestamp, DeviceToken, sign)
	...
	client := &http.Client{Timeout: 30 * time.Second}
	req, _ := http.NewRequest("POST", BaseURL+"/sessions", bytes.NewReader(body))
	for k, v := range headers {
		req.Header.Set(k, v)
	}
...
func checkLogin() bool {
	resp, err := request("GET", BaseURL+"/user?", nil)
	if err != nil {
		return true
	}
...
func main() {
	...
	if _, err := os.Stat(cookieFile); os.IsNotExist(err) {
		...
		headers["cookie"] = string(data)
		fmt.Println("读取到 cookie:", headers["cookie"])
	}
	...
	for checkLogin() {
		...
		cookie, err := getCookie(username, password)
		if err != nil {
